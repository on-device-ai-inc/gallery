...
- **Archived**: 2026-02-17
- **Commits**: `4c18b36`, `980cad8`, `76c13b8`
- **CI Runs**: `22106724534` (Build APK/AAB ✅), `22106724550` (Android CI ✅)
- **App**: Installed and launched on R3CT10HETMM (Samsung S22 Ultra) without crash
- **Key Lesson**: Room tests go in `androidTest/` (not `test/`) — requires Android runtime for SQLite
- **Key Lesson**: KSP version must match Kotlin version exactly: Kotlin `2.1.0` → KSP `2.1.0-1.0.29`
- **Key Lesson**: Target has no separate `DatabaseModule` — DB providers go directly in `AppModule.kt`
- **Key Lesson**: Target has no `ConversationRepository` — DAO injected directly into ViewModels
